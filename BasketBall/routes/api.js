...
const localIP= "http://192.168.0.102";
const redirectURL ="http://localhost:44341/UserInfor/ListOrder"
...
router.get('/backSucccess', async function (req, res) {
	...
	const magiaodich = checkOut.txn_id == undefined ? "": checkOut.txn_id;
	const status = checkOut.stat;
	const checkOutDate = checkOut.updated_at == undefined ? "": checkOut.updated_at;
	console.log(magiaodich,checkOutDate);
	if (status === 'c') {
...
router.post('/getGHN', async function (req, res) {
	...
	const user = (await query.getUserInfoByBillId(billID))[0][0];
	const bill = (await query.getBillAddressById(billID))[0][0].address.split('|');
	//địa chỉ giao hàng hcm/thủ đức/abc
	const billAddress=bill[0];
	// 01/02/03 id của địa chỉ
	const billId = bill[1].split(',');
	...
	var req1 = https.request(options, function (res1) {
		...
		res1.on("end", async function (chunk) {
			...
			if (result.msg === 'Success') {
				result = result['data'];
				console.log("Da Request GHN xong");
	...
	var postData = JSON.stringify({
		...
		"FromDistrictID": 1463,
		"FromWardCode": "21808",
		"ToDistrictID": parseInt(billId[1]),//District ID cua Khach
		"ToWardCode": billId[2].toString(),// Ward ID cua khach
		...
		"ShippingAddress": billAddress,//Dia Chi
		"CoDAmount": 0,//COD
...
router.post('/cartOnline', async function (req, res) {
	//Xu Ly Don Hang
	const cusId = await query.findCusIdByEmail(req.decoded.email);
	const orderinfo = await query.Cart(cusId, JSON.parse(req.body.arrayDetail),req.body.address);
