...
let pool = null; 
require("./dbConnect").then(pool2 => {
    pool = pool2;
});

const getType = function () {
	return new Promise(function (resolve, reject) {
		pool.request().query("select * from product_type;", function (err, rows, fields) {
			if (err) reject(err);
			resolve(rows.recordsets);
		});
	});
}
...
const getTopProduct = function () {
	return new Promise(function (resolve, reject) {
		pool.request().query("SELECT p.id,p.name as name ,p.id_type as idType, t.name as nameType, p.price, p.description, GROUP_CONCAT(i.id) AS imagesID FROM product p LEFT JOIN images i ON p.id = i.id_product inner join product_type t ON t.id = p.id_type group by p.id LIMIT 0,6", function (err, rows, fields) {
			if (err) reject(err);
			resolve(rows.recordsets);
		});
	});
...
const getTypeImage = function (ID) {
	return new Promise(function (resolve, reject) {
		pool.request().query("select image from product_type where id = '" + ID + "';", function (err, rows, fields) {
			if (err) reject(err);
			resolve(rows.recordsets);
		});
	});
...
const getProductByTypeId = function (ID, PAGE) {
	...
	return new Promise(function (resolve, reject) {
		...
		pool.request().query(query, function (err, rows, fields) {
			if (err) reject(err);
			resolve(rows.recordsets);
		});
	});
...
const InsertBill = function (idCustomer, TongTien , address) {
	return new Promise(function (resolve, reject) {
		var query = "INSERT INTO bill(id_customer,date_order, total, address) VALUES ("+idCustomer+", GETDATE(), "+TongTien+",N'"+address+ "') ; SELECT SCOPE_IDENTITY() AS id;";
		
		pool.request().query(query, function (err, result) {
			if (err) console.log(err);
			let insertID = result.recordset[0].id;
			resolve(insertID);
		});
	});
}
...
const Cart = function (idCustomer, listProduct, address) {
	return new Promise(async function (resolve, reject) {
		let TongTien = await TinhTongTien(listProduct);
		let insertID = await InsertBill(idCustomer, TongTien, address);
...
const findCusIdByEmail = function (email) {
	return new Promise(function (resolve, reject) {
		...
		pool.request().query(query, function (err, rows) {
			if (err) console.log(err);
			resolve(rows.recordsets[0][0].id);
		});
	});
}
...
const setCart = function (email, data) {
	return new Promise(async function (resolve, reject) {
		...
		pool.request().query(query, async function (err, rows) {
			if (err) console.log(err);
			else {
				await asyncForEach(data, async (product, index, array) => {
					...
					pool.request().query(query2, function (err, rows, fields) {
						if (err) console.log(err);
						...
						pool.request().query(query, function (err2, rows2) {
...
const getBillAddressById = function(idBill){
	return new Promise(function (resolve, reject) {
		var query = "select address from bill where id ="+ idBill;
		pool.request().query(query, function (err, rows) {
			if (err) console.log(err);
			resolve(rows.recordsets);
		});
...
module.exports = {
	...
	getBillAddressById : getBillAddressById
};